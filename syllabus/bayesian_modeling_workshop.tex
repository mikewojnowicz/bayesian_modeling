...
\subsection{Semi-conjugacy} \label{sec:semi_conjugacy}
...
\begin{itemize}
\item \textbf{Intro to sampling} - Basic sampling methods (inverse cdf,  rejection sampling) and introduction to Markov Chain Monte Carlo (MCMC) - Metropolis Hastings, Gibbs sampling.
\item \textbf{Variational inference} - We will focus on coordinate ascent variational inference (CAVI) \cite{wojnowiczXXXXfoundations}.
\end{itemize} 
...
\item \textbf{Mixture models} - Brief presentation of Gaussian mixture models. Give CAVI.
\item \textbf{Hidden markov models} -  Brief presentation of hidden markov models\footnote{Nice reference for frequentist HMM's: \url{http://jwmi.github.io/ASM/5-HMMs.pdf}}. May give short overview to probabilistic graphical models here.  
\end{itemize}

\subsection{Non-conjugacy} \label{sec:non_conjugacy}
...
\begin{itemize}
	...
	\item \textbf{Variational inference for non-conjugate models} - We will probably need to skip this in the interest of time.
\end{itemize}
\subsubsection{Models}
